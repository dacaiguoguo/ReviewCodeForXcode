...
- (void)asyncFileHandleReadCompletion:(NSNotification *)notif {
    ...
    if ([[notif object] isEqual:[outPipe fileHandleForReading]]) {
        ...
        if (receivedOutputString)
            receivedOutputString([[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
        ...
        if (receivedErrorString)
            receivedErrorString([[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
